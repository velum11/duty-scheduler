...
- Out of scope (stays with `ui-feature` alone): copy/menu wording, permission-unchanged
  pure route wiring, small CSS fixes within existing tokens that change no §1.4 density value or
  action position, single-control alignment.

## Method

1. Read the current screen (code + live rendered observation) and its contracts:
   `DESIGN.md` §0(결정 문장)·§1(토큰)·§2(화면 유형)·§5(검증),
   `views/common/scaffold.py::ARCHETYPES`, the screen manifest,
   `docs/requirements.md` for the affected feature.
2. Apply `screen-design` Stage 0 (intent 3 questions → one focal point → density tier
   from §1.4 → use-what-exists → component checkpoint → states).
3. Self-review the design with `ux-review` (0–4 severity, evidence-tagged). This is a
...
2. **유지할 계약** — exact SoT sections (DESIGN/requirements/code/tests) that must not
   change.
3. **구조 결정** — archetype, area order, split ratios, §1.4 density tier + §1.1 spacing
   values, component
...
- §1 token / §1.4 density gates at design stage are **target checks**; the quantitative PASS is
