---
name: duty-ux
description: Usability (UX) review for duty-scheduler ERP screens and screen designs in this repository — task flow, next-action clarity, form usability, error recovery, status feedback, and cognitive load, scored with a 0–4 severity scale. Preloaded on designated duty-scheduler agents; the narrow duty-scheduler-only wording is to minimize accidental selection elsewhere (it does not technically prevent it).
---

# Duty UX — 사용성 검토 (휴리스틱 평가)

duty-scheduler 화면·설계안을 **사용성 관점**(사용자가 일을 얼마나 쉽게 끝내는가)으로 검토한다.
...
duty-scheduler ERP 맥락으로 재작성했다. 라이선스 전문·허가 문구는 저장소 루트
...
파일을 만들지 않는다(DLP) — 결과는 메시지/handoff 안에 다음 구조로:
...
- 평가 입력은 **설계 서술·코드·라이브 DOM 텍스트**로 충분하다 — 스크린샷 파일을 만들지 않는다.
