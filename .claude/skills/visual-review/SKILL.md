...
2. **타이포그래피** — 위계(§1.2 네 단계 24·20·14·12 — page/section/body/table/label
   과 각 `-strong`), 행 길이·행간, 네 단계 밖 크기나 400·600 밖 굵기가 있는지,
   Pretendard 밖 서체·모노 지정이 남아 있는지, 잘림.
...
5. **컴포넌트 일관성** — 버튼·카드·폼 입력·아이콘·radius·그림자가 화면군 전체에서 단일 어휘인가
   (radius 혼재·카드 남발 = 위반), 공용 키트(`views/common/erp/`) 사용 여부.
