...
| 항목 | 내용 | 출처 |
|---|---|---|
| 사이드바 디자인 확정(sign-off) | 라이트 재설계(테마-ready) 실브라우저 확인 대기. **확정 전엔 DESIGN.md 미반영** — 확정 후에만 §1(사이드바 구조: 검색·접이식 트리·즐겨찾기)·§2(App Shell 사이드바 토큰 다크→라이트 + 테마-ready 명문화)·memory `sidebar-design-protected` 갱신. 상세 메모: `docs/WORKLOG.md` 2026-07-26 항목 "⚠️ 사이드바 확정 후에만 반영할 MD". 2026-07-26 사용자 지시로 미리 적용 금지(확정 디자인 규약화 목적) | 2026-07-26 사용자 |
