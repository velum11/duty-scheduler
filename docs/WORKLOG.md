...
- **타이포 최대 위반 해소**(§7.2-10 1단계): `views/master/style.py`의 `.ms-title` **25px→28px**(-.025em→-.02em), `.ms-desc` **13.5px→12px**. **이 한 선택자가 22개 화면 제목을 전부 지배**했다. 단 "한 곳뿐"은 아니었다 — `views/workspace.py`에 `.ms-title{1.18rem/700}`·`.ms-desc{0.82rem}` **사본**이 남아 있었다(주입 함수 `workspace.master_screen_head` 호출부 0건이라 런타임 영향은 없었으나 구값 재유입 경로였다). 실렌더 확인에서 발견해 제거하고, 사본 재발 방지 검사를 `test_sidebar_ui.py`에 추가했다. `test_sidebar_ui.py`의 값 검증을 함께 갱신하고 **구값(25px/13.5px) 재유입 방지 검사를 추가**했다.
- **실렌더 검증(§5 측정 게이트, 이 프로젝트 최초 실행)**: sample 모드 실행 후 브라우저에서 **대시보드·근무형태 관리·평가 관리**를 **1366×768 / 1024×768 / 375×812** 세 폭으로 계측했다.
  - 제목 **28px/600/-0.56px**(=-.02em), 설명 **12px** — 전 화면 일치. 문서 내 `.ms-title` 규칙은 **1개뿐**(사본 미주입 확인). 구값 25px/13.5px 렌더 요소 **0개**.
  - 1024px에서 **타이틀 밴드 아이콘 9개와 제목 사각형 교차 0건** — `style.py:314`가 기록한 과거 겹침 회귀 없음. 세 폭 모두 **가로 스크롤 없음**, 제목 말줄임 발생 없음.
  - 375px에서 제목 1줄·설명 2줄로 접힘. 평가 관리 렌더 시 **예외 0건**(구 칩 스트립 골격 그대로 — 3단계 미착수 상태 확인).
  - **부수 확인**: IBM Plex Sans KR 400/500/600/700이 실제로 **로드됨**(이 PC 기준). CDN 위험은 사내망에서만 문제이므로 아래 항목 유지.
...
- **§7.1 2단계 완료 — `erp.select_list` 신설**(ORCA, `b72821e`): WORKLIST 목록 행(2줄 고정 + 좌측 2px 상태 보더)을 기존 두 수단이 구조적으로 못 받는다 — `st.button`은 label이 inline 요소만 허용하고, `select_grid`는 `_SELECT_COL_CONFIG_ALLOWED`가 `cellRenderer`를 **의도적으로** 막는다(SELECT는 편집 capability가 아니라는 §0.5 계약). **그 차단을 뚫지 않고** `st.components.v2` 패턴으로 목록 전용 컴포넌트를 새로 만들었다.
  - 계약: 선택이 **위치가 아니라 자연키**로 오간다(정렬·필터가 바뀌어도 같은 건이 열림). 상태 보관은 호출부 책임. 선택 표시는 `select_grid`와 같은 어휘(틴트 + 좌측 바), 색 단독 아님.
  - `test_erp_select_list.py` 23건 신설. **WORKLIST 4화면이 공유하므로 3단계 골격 이관의 선행 조건**이었고, 이제 3단계 즉시 착수 가능하다.
- **스킬·에이전트 문서 인용 정정**(ORCA, `0cbaa84`): 폐기된 §0.6·§0.3·§8을 인용하던 **6개 파일 15곳**을 교체했다(인계 메모의 "screen-design 5곳"은 과소 집계였다). **번호만 바꾼 게 아니다** — 히트영역이 구 "≥32px 단일 하한"에서 §1.4 밀도 등급별(cozy ≥44 / compact 34~38 / condensed 24~28)로 바뀌었고 **condensed는 터치 대상이 아니라는 새 규정**이 있어, 옛 문구를 두면 감사 담당이 틀린 기준으로 측정한다. 구 고정값("조건영역 ≤96px·상세 ≥600px")도 §1.1 스케일로 교체.
  - 같은 커밋에서 `master/style.py` 라벨 하한 위반 수정 — `.ms-chip`만 12px로 올리고 형제 라벨 6개(9.8~10.4px)가 미달로 남아 있었다(`.ms-empty`는 5화면 공유).
  - 인계 메모 오기 정정: `references/`는 **깨진 참조가 아니다.** 참조처는 `test-selection` 하나뿐이고 그 파일은 실재한다.
- **BACKLOG로 넘긴 것 2건**: ① 공용 버튼·입력 `min-height:2.15rem`(=**30.1px**)이 §1.4 compact 하한(34px) 미달 — 이미 5곳 이상이 개별 우회 중이라 공용 기본값이 틀렸다는 신호지만 **버튼 높이가 인접 그리드 행·조건줄에 연쇄**하므로 3단계 visual-qa 실측과 함께 처리한다. ② 경계선 폰트 5곳(10.92px, 하한에 0.08px 미달) — 폭 연쇄 리스크가 이득보다 커서 보류.
- **미커밋으로 남은 것(주의)**: `views/common/proto.py`에 병행 세션 작업(+59줄)과 **ORCA의 1줄 수정이 섞여** 있다. `MONO` 상수가 작은따옴표를 품어 어제 아차사고 6화면에서 고친 것과 **같은 `style` 소실 버그가 숙소예약·업무요청 3화면에 남아 있던 것**을 고친 줄이다. hunk 분리가 CRLF 문제로 실패해 커밋되지 않았다 — **병행 작업 소유자가 커밋할 때 이 줄이 함께 들어가야 하며, 빠지면 같은 시각 결함이 되살아난다.** `views/lodging_request.py`(+271줄)도 미커밋 보존 중이다.
...
- **근무표 3화면**(오전 병렬분): 편성·월간 대분류/중분류 축, 부서 필터 대분류화([전체·PET생산부·PVC생산부], 월 편성 유도), 정렬 대분류→조(A→B→C)→등록순, 월간 조회버튼 제거+헤더 새로고침, 일자열 40px 밀도, 내근무표 한 줄 배치(`width="content"` 계약).
- **편성 저장 후 공백 버그**: 성공 배너의 최상위 유무 → delta 경로 밀림 → 직전 그리드 블록 잔존. 배너·삭제경고·단축키를 상시 존재 `se_notice` 컨테이너로 흡수(요소 수 고정). 같은 유형 잠재 결함이 다른 화면에도 있을 수 있음(미확인, 백로그).
- **내 아차사고**: 아코디언 → 조회 동형 컬럼 표(신고자·소속 제외) + 전체폭 상세. **상태 열은 발생일 뒤로 전진**(모바일 첫 화면 노출, 코덱스 자문).
- **아차사고 분석 KPI**: 3줄→2줄(영문 오버라인 제거), 누적·당월 동일 grid 트랙, 카드 92→69px.
- **월간 근무표**: 첫 진입 디폴트=내 부서·조(편성 스냅샷 shift→team→마스터 3단 폴백, 빈 위젯만 시드), 좁은 폭 성명+일자만(CCv2 뷰포트 프로브로 판정, CSV 불변), 세로 회전 안내+가로 크롬 압축.
- **원인 라벨 정본 통일(결함 수정)**: 조회·평가·개선조치·분석의 JAM/PINCH 가 등록과 뒤바뀜 → **JAM=끼임, PINCH=협착**(KOSHA 용법, 코덱스 자문)으로 4화면 교정. DB 코드값이라 데이터 무변경.
- **브랜딩 확정(코덱스 자문)**: 사이드바 `WorkOps`+부제 `현장운영`, 탭 제목 `WorkOps · 현장운영`, 파비콘 오렌지 W 재생성. test_sidebar_ui 106.
- 대시보드 모바일: 주간·야간·휴무 3열 백분율 basis 로 줄바꿈 해소(≤640px).
- 아차사고 샘플 10건+CC 사진(Openverse) 적재 — 보고자 전원 ADMIN(실직원 미귀속), CLOSED 제외(CAPA 폐루프 계약). 로더 `.orca/artifacts/near-miss-sample/`.
- 운영 메모: Cloud 는 push 자동배포가 오늘 전혀 안 붙어 **매번 수동 Reboot 필요**했음. 로컬 8501-8503 은 하네스 Job Object 에 딸려 죽어 **WMI Win32_Process.Create(+cd /d)** 로 독립 기동(메모리 기록).
- 미결: 편성 조회 조 필터에 실데이터 `A팀`/`B팀` 노출(원료1파트 2명 편성값 — 데이터 교정 시 자동 소멸), 대분류 옵션 월 종속 vs 마스터 고정, USER 역할 월간 강제 새로고침 진입점 없음, DESIGN.md §0 매니페스트 부재(에이전트 3명 공통 보고).

## 2026-08-13 · [베타] 일반 사번 로그인 오픈 — 008 적용 + 강제 비번변경 한시 해제 (사용자 지시)

- **migration 008 적용 완료**(테스트 프로젝트, MCP apply_migration): users 비번 컬럼 6종 + login_sessions. 기존 행 무변경(해시 NULL = 초기 비번이 곧 사번).
- **`config.FIXED_PASSWORD_ACCOUNTS` 비움**(베타 전 필수 항목 이행) — ADMIN 도 일반 경로(초기 비번 = 사번 `ADMIN`)로 로그인.
- **`config.BETA_SKIP_FORCED_PASSWORD_CHANGE = True` 신설(한시)**: 강제 비밀번호 변경 **라우팅 게이트만** 끔(auth.needs_password_change). 로그인 검증·연속실패 잠금·세션 계약은 그대로. **정식 전환 시 False 로 되돌리면 강제변경 복원** — 그 전까지 전 직원이 "사번=비밀번호" 상태로 사용(보안상 한시적 타협임을 인지하고 승인됨).
- login 화면 안내 문구를 스위치에 연동. `test_password_auth` 56 PASS(베타 스위치 섹션 8 신설), `test_login_auth` 30 PASS.
- 같은 날: 아차사고 샘플 10건+CC 사진 적재(`.orca/artifacts/near-miss-sample/`), 베타 안내 메일 워드(`.orca/artifacts/beta-mail/`, 스샷 가명 치환).
