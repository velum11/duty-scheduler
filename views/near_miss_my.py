...
_LIST_COLUMNS = ["보고번호", "작업명", "발생일", "제안등급", "상태"]
_LIST_COL_CONFIG = {
    "보고번호": {"width": 118},
    "작업명": {"flex": 2, "minWidth": 140},
    "발생일": {"width": 104},
    "제안등급": {"width": 82},
    "상태": {"width": 88},
}
...
def _list_display(reports: pd.DataFrame) -> pd.DataFrame:
...
def _render_list(reports: pd.DataFrame) -> None:
    display = _list_display(reports)
    st.markdown(
        f"<div style='font-weight:600;color:{TOKENS['ink']};margin:2px 0 6px;'>"
        f"내 아차사고 목록 "
        f"<span style='color:{TOKENS['ink-2']};font-weight:400;'>· 총 {len(display)}건</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
    status_rules = {label: _STATUS_COLOR[code] for code, label in _STATUS_LABEL.items()}
    erp.read_grid(
        display, columns=_LIST_COLUMNS, key=f"{_PAGE_ID}_grid",
        col_config=_LIST_COL_CONFIG, color_rules={"상태": status_rules},
    )


# ---------- 상세(details) + SUBMITTED 자기수정 ----------
def _report_options(reports: pd.DataFrame) -> list[str]:
    frame = reports.sort_values("incident_date", ascending=False, kind="stable")
    return [str(r.get("id")) for _, r in frame.iterrows()]


def _render_detail(user: dict, reports: pd.DataFrame) -> None:
    options = _report_options(reports)
    labels = {
        str(r.get("id")): f"{_clean(r.get('report_no')) or '(번호 미상)'} · "
        f"{_STATUS_LABEL.get(_clean(r.get('status')), _clean(r.get('status')))}"
        for _, r in reports.iterrows()
    }
    prev = st.session_state.get(_SEL_KEY)
    index = options.index(prev) if prev in options else 0
    selected_id = st.selectbox(
        "상세 볼 보고서", options, index=index, key="nm_my_pick",
    ...
    if match.empty:
        erp.detail_empty("보고서를 선택하세요", "왼쪽 목록에서 확인할 아차사고를 고르세요.")
        return
    ...
    st.caption(
        f"발생원인 {escape(_CAUSE_LABEL.get(cause, cause) or '-')}"
        + (f" · {escape(cause_detail)}" if cause_detail else "")
    )
    st.text_area("작업내용", value=_clean(report.get("work_content")),
                 height=68, disabled=True, key=f"nm_my_wc_{selected_id}")
    st.text_area("사고내용", value=_clean(report.get("incident_content")),
                 height=88, disabled=True, key=f"nm_my_ic_{selected_id}")
    st.text_area("예방대책", value=_clean(report.get("countermeasure")),
                 height=68, disabled=True, key=f"nm_my_cm_{selected_id}")
