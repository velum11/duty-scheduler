...
- **라벨 열 + 값 열**(§3.1·§4-4) — 라벨을 좌측 130px 열로 전치하고 값을 한 열에 모은다.
  ...
  두고 **컨트롤 폭은 그 칸에 들어갈 값의 실측 길이·가독 상한에서 역산**한다(아래 `_KO_ADV_PX`
  주석의 계측 근거). 폭은 `min(100%, 상한)` 이라 좁은 화면에서는 줄고 넓은 화면에서는
  상한에서 선다 — 상한이 없으면 폭이 답의 길이를 말해주지 않고, 상한이 고정폭이면 넓은
  화면에서 폼이 화면과 무관해 보인다(2026-08-19 두 지적을 함께 만족시키는 형태).
...
# ── 글자 폭 계수 — **추정치가 아니라 실측치**(2026-08-19 Pretendard 재계측) ──────
# §1.2 가 2026-08-19 개정으로 글꼴을 **Pretendard 하나**로 통일하면서 종전 IBM Plex 기준
# 계수(한글 12.488 / 라틴 7.952)는 폐지됐다. 아래는 그 자리에서 다시 잰 값이다.
# 계측: sample 모드 :8554(등록 화면 단독 프리뷰) / Chromium 1440×900 / `document.fonts.ready`
# + `fonts.load('400 14px Pretendard','한글')` 뒤, 등록 폼의 **실제 컨트롤**(textarea·input)의
# computed style(`14px/23.8px Pretendard`)을 그대로 복사한 hidden span 에 100자 반복 →
# ÷100(단일 글자 측정의 반올림 오차 제거).
#   한글 '가'×100 = 1210.0px → 12.10px/자   (= 0.864em)
#   라틴 'n'×100  =  779.0px →  7.79px/자   (= 0.556em)
#   숫자 '8'×100  =  849.0px →  8.49px/자
# → **한글 1자 = 라틴 소문자 1.553자 폭**(12.10 / 7.79). 폭 역산은 전부 한글 기준으로 한다.
# 같은 세션에서 크기·굵기를 바꿔가며 재면 한글 advance 는 **0.8643em 로 일정**했다
# (12·14·20px × 400·600 전부 동일 — Pretendard 한글 글리프 폭이 굵기에 불변). 즉 12px 이면
# 10.37px/자다. 다른 곳에 적힌 "12px 한글 11.04(=0.92em)"는 이 스택·이 계측법으로 재현되지
# 않으므로(자간이 얹힌 계측으로 보인다) 폭 역산에는 **이 화면 컨트롤에서 잰 값**만 쓴다.
_KO_ADV_PX = 12.10
_LAT_ADV_PX = 7.79           # 한글/라틴 계수 = 12.10 / 7.79 = 1.553

# ── 컨트롤 여백 실측(같은 계측 세션) ──
#   text_input  : 좌 10.5 / 우 7,   테두리 1px      textarea : 상하좌우 10.5, 테두리 1px
#   date_input  : 좌 11.25 / 우 7 + 래퍼 우 3.5,    selectbox: 좌우 7 + 셰브런 버튼 28
#
# ── 폭 3계단 — 상한은 "그 칸에 들어가는 값 + 가독 상한", 실폭은 화면 폭을 따라간다 ──
# 2026-08-19: 상한을 **고정폭**으로 쓰던 것을 `min(100%, 상한)`(짧은 두 계단은 `clamp(하한,
# 비율, 상한)`)으로 바꿨다. 고정폭은 1440 에서 값 열 오른쪽을 500px 넘게 비워 폼이 화면과
# 무관해 보였고, 그렇다고 전폭으로 되돌리면 "폭이 답의 길이를 말해주지 않는" 종전 결함이
# 되살아난다. 상한은 유지하되 **상한이 너무 낮았던 것**을 아래 가독 상한 근거로 올린다.
# 세 계단은 **같은 배율로 함께** 커진다(계단 비율 유지 — 부류가 섞이면 우측 가장자리가
# 다시 제각각이 된다). 배율 k = 608/524 = 1.1603 이고 4px 격자로 반올림했다:
#   140 → 164(162.4)  /  272 → 316(315.6)  /  524 → 608
#   비율 1 : 1.927 : 3.707 (종전 1 : 1.943 : 3.743, 오차 1% 미만)
# 실값 코퍼스는 이 도메인의 실제 문장이다(modules/db.py `_NEAR_MISS_SEED` = 가상 실사례,
# 그리고 이 화면의 placeholder 예시). 글자 수는 글꼴과 무관하므로 종전 계측의 **자 수**를
# 그대로 쓰고 렌더 폭만 Pretendard 계수(12.10)로 다시 환산한다:
#   사고내용 최장 "호이스트로 포대 인양 중 … 작업자 1m 옆 통과"  한글 31.9자 = 386.0px
#   예방대책 최장 "결속 상태 2인 상호 확인 후 인양, …"          한글 25.9자 = 313.4px
#   현장 상황 최장 "원료 투입장 호이스트 하부"                  한글 11.8자 = 142.7px
#   작업내용 최장 "PET 원료 포대 투입 작업"                     한글 11.0자 = 133.5px
#   작업명   최장 "3라인 컨베이어 벨트 점검"(placeholder 예시)   한글 11.5자 = 138.8px
#   원인 상세 최장 "시야 미확보"                                한글  5.3자 =  63.7px
#   원인 코드 최장 라벨 "미끄러짐 (SLIP)"                        실측 89.31px
#   발생일   "2026-08-05"                                       실측 79.00px
#
# (A) 고정 형식 짧은 값 — 발생일·원인 코드. 상한은 배율(140→164), **하한은 값이 잘리지
#     않는 최소폭**이다. 코드: 89.31 + 좌우 패딩 14 + 셰브런 28 + 테두리 2 = 133.31,
#     날짜: 79.00 + 11.25 + 7 + 래퍼 3.5 + 2 = 102.75 → 부류 최대 133.31 을 4px 격자로
#     올려 하한 136. 둘은 같은 부류라 **같은 폭**을 쓴다(우측 가장자리 한 곳).
_W_SHORT = 164
_W_SHORT_MIN = 136
# (B) 한 줄로 끝나는 구 — 작업명·원인 상세. 상한 316 = (316 − 19.5) / 12.10 = 24.5자,
#     관찰 최장 11.5자의 2.1배 여유이고 서술(48자)의 절반이라 **폭이 "한 줄이면 된다"고
#     말한다**. 하한은 종전 20자 계단 272(= 20 × 12.10 + 17.5 = 259.5 를 격자 올림)로 둔다.
_W_LINE = 316
_W_LINE_MIN = 272
# (C) 서술 — 사고내용·작업내용·현장 상황·예방대책. **가독 상한에서 나온 값**이다.
#     라틴 45~75자 권장의 **상단(75자)**을 위 Pretendard 실측 계수로 환산한다:
#       75자 × 7.79(라틴 'n') = 584.25px 의 글줄 → ÷ 12.10 = 한글 48.3자
#       상자 = 584.25 + 좌우 패딩 21 + 테두리 2 = 607.25 → 4px 격자 올림 = 608
#     종전 524 는 같은 환산에서 40자(라틴 62.1자 상당)로 권장 범위의 중간이었다. 폭 상한
#     자체는 옳았고 **상한이 낮았던 것**이라, 권장 범위를 벗어나지 않는 최댓값(75자)까지만
#     올린다. 최장 실사례 31.9자는 한 줄에 들어가고 34% 여유가 남는다.
#     (전폭 1021px = 한글 84.4자 = 라틴 131자 상당으로 권장의 1.75배였다 — 되돌리지 말 것.)
_W_PROSE = 608
# 화면 폭을 따라가는 실폭 — 값 열 폭에 대한 비율. 상한에서의 비율을 그대로 쓴다
# (164/608 = 26.97% → 27%, 316/608 = 51.97% → 52%). 값 열이 상한보다 좁아지면 세 계단이
# **같은 비율로 함께** 줄어 계단 구조가 유지된다.
_PCT_SHORT = "27%"
_PCT_LINE = "52%"
...
# ── 계단 ↔ 필드 대응은 여기 한 곳에서만 정한다 ──
# 위젯 인자(`width=_W_*`)는 CSS 가 죽었을 때의 폴백(=상한)이고, 실제 폭은 아래 CSS 가
# `min(100%, 상한)` / `clamp(하한, 비율, 상한)` 으로 화면 폭에 맞춘다. 두 곳이 갈라지지
# 않도록 계단 상수를 공유하고, 필드 배정은 이 튜플이 단일 출처다.
_W_TIERS: tuple[tuple[str, tuple[str, ...]], ...] = (
    (f"clamp({_W_SHORT_MIN}px, {_PCT_SHORT}, {_W_SHORT}px)", ("incident_date", "cause_code")),
    (f"clamp({_W_LINE_MIN}px, {_PCT_LINE}, {_W_LINE}px)", ("work_name", "cause_detail")),
    (f"min(100%, {_W_PROSE}px)",
     ("incident_content", "work_content", "site_description", "countermeasure")),
)


def _tier_css() -> str:
    """계단별 폭 규칙 — 위젯 키 클래스(`st-key-nm_f_*`)에 직접 건다.

    선택자 특정도를 낮게(클래스 1개) 유지하는 것이 계약이다. `<599px` 접힘 규칙은
    `[class*="st-key-nmrow_"] div[data-testid="stElementContainer"]`(특정도 더 높음)이라
    접힌 폭에서는 접힘 규칙이 이겨 값 열이 하나로 남는다. Streamlit 이 `width=` 로 넣는
    폭은 생성 클래스(!important 아님)라 !important 하나로 덮인다(실측).
    """
    return "\n".join(
        ",\n".join(f".st-key-{_WKEY}{k}" for k in keys)
        + f" {{ width:{expr} !important; max-width:100% !important; }}"
        for expr, keys in _W_TIERS
    )


_FORM_CSS = f"""
<style>
/* ── 폼 블록 중앙 정렬(2026-08-19) ──
   값 폭에 상한이 있는 한 1440 본문(1405px)은 절대 채워지지 않는다. 좌측 정렬로 두면 남는
   651px 이 전부 오른쪽에 몰리고, 그 빈 폭 위로 신원 줄의 안내("로그인 정보로 자동 지정"),
   섹션 헤어라인, 하단 바의 [제출]이 끌려가 **폼에서 650px 떨어진 곳에 주 액션이 놓인다**
   (실렌더 비교 A/B 확인). 본문 블록 자체를 폼 폭으로 좁혀 가운데 두면 제목·신원·섹션 선·
   제출이 한 세로 띠 안에 남는다.
   폭 = 라벨 130 + 열 간격 16 + 값 상한 608 = 754, + 블록 좌우 패딩 17.5×2 = 789 →
   8px 격자 792 + 스케일 한 칸 8 = 800. 남는 11px 은 섹션 헤어라인이 서술 상자 오른쪽
   끝에 딱 붙어 보이지 않게 하는 여유다.
   `:has(st-key-nm_css)` 로 **이 화면이 그린 본문에만** 건다(nm_css 주입 컨테이너는 등록
   폼과 제출 완료 화면 양쪽에서 항상 렌더된다). 좁은 폭에서는 max-width 가 본문보다 커서
   아무 효과가 없다 — §3.4 접힘 동작은 그대로다. */
section[data-testid="stMain"] div[data-testid="stMainBlockContainer"]:has([class*="st-key-nm_css"]) {{
...
/* ── 값 컨트롤 폭 — 화면 폭을 따라가되 가독 상한에서 멈춘다(2026-08-19) ──
   `min(100%, 상한)`. 값 열이 상한보다 넓으면 상한에서 서고, 좁아지면 함께 줄어든다.
   짧은 두 계단은 `clamp(하한, 비율, 상한)` 이라 **세 계단이 같은 비율로** 줄어든다 —
   비율이 무너지면 "짧은 값 / 한 줄 답 / 서술"이라는 부류 구분이 폭에서 사라진다.
   하한은 값이 잘리지 않는 최소폭이다(_W_SHORT_MIN·_W_LINE_MIN 주석의 역산). */
{_tier_css()}
/* 위젯 래퍼도 같은 칸 폭을 채운다 — Streamlit 이 `width=` 로 넣는 고정폭이 래퍼 단계에도
   걸려 있어 칸만 넓히면 컨트롤이 따라오지 않는다(실측). */
[class*="st-key-nmrow_"] div[data-testid="stTextInput"],
[class*="st-key-nmrow_"] div[data-testid="stTextArea"],
[class*="st-key-nmrow_"] div[data-testid="stDateInput"],
[class*="st-key-nmrow_"] div[data-testid="stSelectbox"] {{ width:100% !important; }}
...
@media (max-width:598px) {{
  ...
  /* 폭 3계단(상한 164/316/608)은 넓은 폭의 값이다. 접힌 폭에서는 값 열 자체가 이미 좁으므로
     컨트롤을 열 폭에 맞춘다 — 그러지 않으면 375px 화면에서 짧은 두 계단의 하한(136/272)이
     남아 가로 스크롤이 생긴다. 이 규칙은 계단 규칙(_tier_css, 클래스 1개)보다 특정도가
     높아 접힌 폭에서 반드시 이긴다. 값 열은 접힌 뒤에도 **하나**다(§3.1). */
...
def render(user: dict) -> None:
    ...
    with _row("work_name", "작업명"):
        # 폭 상한 316 = 한 줄 구 24.5자(_W_LINE 주석의 역산). 작업명 실값은 "원료 투입"(5자)~
        # "3라인 컨베이어 벨트 점검"(11.5자 등가)이라 넘치지 않고, 값 열을 다 채우지
    ...
    _section("02", "상황 기술")
    # 서술 4칸은 같은 폭 상한(608 = 한 줄 48자)을 쓴다 — 같은 부류의 칸이 서로 다른 폭이면
    ...
    with _row("cause_detail", "발생원인 상세"):
        ...
        # 답 길이가 아니므로 폭은 한 줄 구 계단(상한 316)을 쓴다.
